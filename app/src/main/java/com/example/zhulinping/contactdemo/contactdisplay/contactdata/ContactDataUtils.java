...
import com.example.zhulinping.contactdemo.contactdisplay.contactdata.model.ContactDbInfo;
...
public class ContactDataUtils {
    ...
    public static void fillContactInfos(Context context, List<ContactInfo> source) {
        ...
        final String[] projection = new String[]{ContactsContract.CommonDataKinds.Phone.CONTACT_ID, ContactsContract.Data.MIMETYPE,
        ...
        ContentResolver contentResolver = context.getContentResolver();
        Map<String, List<ContactInfo.TypeInfo>> phoneMap = new HashMap<>();
        Map<String, List<ContactInfo.TypeInfo>> emailMap = new HashMap<>();
        ...
        try {
            ...
            while (phoneCursor.moveToNext()) {
                ...
                int valueType = phoneCursor.getInt(3);
                switch (type) {
                    case TYPE_EMAIL: {
                        emailNum = value;
                        if (emailMap.containsKey(contactId)) {
                            ContactInfo.TypeInfo info = new ContactDbInfo.TypeInfo();
                            info.type = valueType;
                            info.value = emailNum;
                            emailMap.get(contactId).add(info);
                        } else {
                            List<ContactInfo.TypeInfo> list = new ArrayList<>();
                            ContactInfo.TypeInfo info = new ContactDbInfo.TypeInfo();
                            info.type = valueType;
                            info.value = emailNum;
                            list.add(info);
                    ...
                    case TYPE_PHONE: {
                        ...
                        if (phoneMap.containsKey(contactId)) {
                            ...
                            info.type = valueType;
                            info.value = phoneNum;
                            phoneMap.get(contactId).add(info);
                        } else {
                            List<ContactInfo.TypeInfo> list = new ArrayList<>();
                            ContactInfo.TypeInfo info = new ContactDbInfo.TypeInfo();
                            info.type = valueType;
            ...
            for (ContactInfo bean : source) {
                ...
                if (TextUtils.isEmpty(bean.contactName)) {
                    if (bean.phoneNumList != null && bean.phoneNumList.size() > 0) {
                        bean.contactName = bean.phoneNumList.iterator().next().value;
